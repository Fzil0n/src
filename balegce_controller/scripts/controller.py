...
class controller(Node):
    ...
    # Controller ---------------------------------
    def roll_PDcontroller(self,error:float, error_dot:float, threshole:float)->float:
        if(error >= threshole):
    ...
    def yaw_PDcontroller(self, error:float, error_dot:float, threshole:float)->float:
        if(abs(error) >= threshole):
